...
def _extract_product_fundings(db, product, start_date, end_date, pbar, data_path, is_local, rotation=False):
    ...
    while (end_date - fetch_date).days > 0:
        ...
        path = f"data/futures/{market}/monthly/fundingRate/{symbol}/" if not is_month_fully_completed else f"data/futures/{market}/daily/fundingRate/{symbol}/"
        next_fetch_date = fetch_date + r_delta
        if is_local==False and not utils.is_already_saved(data_path, symbol, fetch_date, mode=utils.SavedModes.FUNDING, is_month_fully_completed=is_month_fully_completed, is_local=False):
            utils.get_binance_data(path, db, fetch_date, next_fetch_date, data_path, is_local, rotation=rotation)
        elif not utils.is_already_saved(data_path, symbol, fetch_date, mode=utils.SavedModes.FUNDING, is_month_fully_completed=is_month_fully_completed, is_local=True):
            utils.get_binance_data(path, db, fetch_date, next_fetch_date, data_path, is_local, rotation=rotation)
